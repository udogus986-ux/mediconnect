  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // OpenStreetMap Overpass API ile yakındaki hastane/klinik bul
        try {
            ...
            {[
              ...
              <button key={f.key} onClick={() => setSelectedType(f.key as any)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                  selectedType === f.key ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant hover:bg-secondary-container'
                }`}>
                {f.label}
              </button>
            ))}
      ...
      <div style={{ height: 380 }}>
        ...
        >
          ...
          <Marker position={[userLocation.lat, userLocation.lng]} icon={userIcon}>
            <Popup>
              <div className="text-sm font-medium">📍 Konumunuz</div>
            </Popup>
          </Marker>
              ...
              <Popup>
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {place.type === 'hospital' ? '🏥 Hastane' : '🏪 Klinik'}
                  </div>
